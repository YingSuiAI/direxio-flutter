...
- Agent conversation headers show user-facing online state from `sync.bootstrap.agent_online` and live owner `GET /_p2p/events` `agent.presence` events whose payload contains only `online`. Reply generation/typing is shown separately from online/offline presence.
