...
//
// Matrix SDK's mesh backend normally sends group-call SDP/ICE through to-device
// events. Our current Dendrite-based multi-node deployment reliably federates
// room events, while direct-to-device signaling is not stable enough for a
// production MVP. The product currently supports one active app device per user,
// so room events with invitee user/device fields are an acceptable transport
// boundary until we move group calls to a dedicated MatrixRTC/SFU solution.
const bool p2pForceGroupCallRoomEvents = true;
...
class CallSession {
  ...
  Future<void> onAnswerReceived(
    ...
    if (direction == CallDirection.kOutgoing) {
      ...
      final pendingRemoteCandidates = _remoteCandidates.toList(growable: false);
      for (final candidate in pendingRemoteCandidates) {
  ...
  Future<String?> _sendContent(
    ...
    if (isGroupCall && remoteDeviceId != null && !p2pForceGroupCallRoomEvents) {
