...
class VoIP {
  ...
  Future<void> _handleCallEvent(BasicEventWithSender event) async {
    ...
    if (event is Event) {
      ...
      /// For P2P IM's room-event group-call transport, group-call invites carry
      /// the sender device in `device_id` and the target device in
      /// `invitee_device_id`. Track the remote caller device when present so
      /// answers and duplicate-call checks bind to the actual peer.
    ...
    if (event is Event &&
        content.tryGet<String>('conf_id') != null &&
        remoteUserId == client.userID) {
      Logs().v(
          'Ignoring room-event group call event ${event.type} for room ${room?.id} from our own user');
      return;
    }
