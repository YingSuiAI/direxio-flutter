...
class GroupCallSession {
  ...
  Future<void> onMemberStateChanged() async {
    ...
    final p2pMembershipsToSetup = <MapEntry<CallParticipant, CallMembership>>[];
    ...
    for (final mem in memsForCurrentGroupCall) {
      ...
      if (oldPcopy.contains(rp)) continue;

      p2pMembershipsToSetup.add(MapEntry(rp, mem));
    }

    for (final membershipToSetup in p2pMembershipsToSetup) {
      await backend.setupP2PCallWithNewMember(
        this,
        membershipToSetup.key,
        membershipToSetup.value,
      );
    }
