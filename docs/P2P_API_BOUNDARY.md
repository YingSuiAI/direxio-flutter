...
- Block actions: `blocks.list`, `blocks.add`, and `blocks.remove`. The blacklist is contact-only: `blocks.list` returns `contacts`, and contact blocks are keyed by peer Matrix id. Block entries must include `display_name` and may include `avatar_url` so the blacklist UI can show recognizable names instead of ids.
- The client hides blocked contacts and matching direct conversation rows from normal lists. Other entry routes, including contact/profile and direct-room routes, must intercept blocked contacts and show an already-blocked prompt. Unblock is only exposed from Settings -> Blacklist; ordinary contact settings pages keep only the block action. Group and channel settings pages do not expose blacklist actions.
