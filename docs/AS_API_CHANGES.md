...
- Product conversation route generation now fails closed when `canOpen` is false, so message list, contact detail, search, channel tab, and group entry points no longer independently infer whether a conversation can be opened.
- `groups.create`, `groups.join`, and `channels.join` may return top-level `conversation` with the same `ConversationView` shape. `AsGroupResult.productConversation` and `AsChannel.productConversation` preserve that object.
- Group creation, group invite acceptance, channel invite/share acceptance, public channel chat joins, and channel tab entries open chats through ProductCore conversation routes. If the conversation is missing or not openable, the client waits on the current screen instead of reconstructing a chat route from channel id, group name, or member count.
