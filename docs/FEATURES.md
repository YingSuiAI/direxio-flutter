...
- Agent chat headers expose an Agent settings entry. The settings page reads and updates `agent.config`, including Agent nickname, `avatar_url`, and `mcp_blocked_room_ids`; the room picker is populated from current direct/group/channel conversations and already blocked rooms are selected.
