...
class ChatScrollToLatestCoordinator {
  ...
  bool shouldRetryAfterAttempt(
    Object itemKey, {
    required bool hasPosition,
    required bool isAtLatest,
    required bool allowSettling,
    int maxAttempts = chatLatestInitialAutoScrollMaxAttempts,
    int settleAttempts = chatLatestInitialAutoScrollSettleAttempts,
  }) {
    if (_pendingItemKey != itemKey) return false;
    if (_retryAttempts >= maxAttempts) return false;
    final shouldRetry = !hasPosition ||
        (allowSettling && (!isAtLatest || _retryAttempts < settleAttempts));
