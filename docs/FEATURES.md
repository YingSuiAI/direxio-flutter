...
- Agent conversation headers show user-facing online state from `sync.bootstrap.agent_presence.online` and live owner `GET /_p2p/events` `agent.presence` events. `connected` is only the agent-token SSE connection fact; reply generation/typing is shown separately from online/offline presence.
