...
| Direct conversation list | `home_page.dart`, `home_conversation_snapshot_store.dart` | Real + local | ProductCore conversations are the logged-in conversation source. The home list may render a local snapshot only while ProductCore is still loading after app restart; after ProductCore returns, snapshot rows that are not in ProductCore are pruned, while same-room cached preview fields can still fill transient Matrix hydration gaps. Opening a real conversation uses the ProductCore `conversation_id` plus `matrix_room_id`, not local room-type guessing. Locally hidden rows reappear when opened from contact detail or when new unread activity arrives. |
...
| Friend/group/channel pending requests | `/requests`, `requests_page.dart` | Real | Uses AS pending state and Matrix native direct profile metadata; new direct invites do not require legacy `p2p.contact.request`. AS `pending.group_invites` are discoverable from the New Friends entry and can be accepted through AS group join. Successful group joins open through the returned ProductCore conversation, not a locally reconstructed group route. |
